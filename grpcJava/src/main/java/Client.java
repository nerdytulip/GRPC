...
public class Client {
    ...
    private void ArmstrongNumber(ManagedChannel channel) {
        CalculatorServiceGrpc.CalculatorServiceBlockingStub stub = CalculatorServiceGrpc.newBlockingStub(channel);

        CalculatorRequest req = CalculatorRequest.newBuilder().setNumber(153).build();

        ArmstrongNumberResponse res = stub.armstrongNumber(req);

        System.out.println("armstrong: " + res.getResult());
    }
